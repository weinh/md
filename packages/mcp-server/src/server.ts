/**
 * Shared, side-effect-free building blocks for the MCP server entries.
 *
 * This module MUST NOT connect a transport or call server.connect() at the top
 * level — that way multiple thin entries (index.ts, index-file.ts, …) can import
 * the reusable pieces and register their own `render_markdown` variant without
 * accidentally spawning servers.
 */
...
export const SERVER_NAME = `md-mcp-server`
...
/**
 * All `render_markdown` option fields EXCEPT the source field (`markdown`/`file`).
 * Entries compose their own source field on top of this shared set so the option
 * surface stays identical across variants.
 */
export const renderOptionFields = {
  theme: z
...
/** Register every read-only list/explain tool. Shared by all server entries. */
