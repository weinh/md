...
/**
 * Install production runtime deps into plugin/renderer/runtime for
 * self-contained .sublime-package packaging. The esbuild runtimeExternalPlugin
 * resolves isomorphic-dompurify from this folder at runtime.
 *
 * Adapted from apps/vscode/scripts/copy-runtime-deps.mjs.
 */
...
const runtimeDir = path.join(pkgRoot, `plugin`, `renderer`, `runtime`)
...
function resolveDompurifyVersion() {
  ...
  let dir = path.dirname(requireFromPkg.resolve(`isomorphic-dompurify`))
...
console.log(`✓ runtime deps ready at ${path.relative(pkgRoot, path.join(runtimeDir, `node_modules`))} (isomorphic-dompurify@${dompurifyVersion})`)
